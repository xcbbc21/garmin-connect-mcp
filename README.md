...
| `GARMIN_ACCOUNT` | ❌ | Lowercase local alias used for the default Web-login session path (`default` when omitted) |
...
> or `GARMIN_SESSION_TOKEN_FILE`. The experimental local Web login may start
> without one and creates the default account session file. A protected file is safer than an inline token,
