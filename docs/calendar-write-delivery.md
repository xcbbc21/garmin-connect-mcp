...
> ## ⚠ 续修更新（2026-09-12，代码与测试树截至 `db66d01`；其后提交仅改文档，平台重测基于 `73e16a2` 树）
>
...
> | "三平台验证：仅 macOS；Linux/Windows 未跑，CI 也未接入新用例" | CI 平台作业**已接入**并覆盖上述恢复用例。**实测结果（全部在 `73e16a2` 的代码树上重测，该树与当前 HEAD 的 `src/`+`tests/` 逐字节一致）：** macOS 本机完整套件 **59 套件 / 1215 用例（1215 通过、0 跳过）** exit 0，本轮新增的锁用例 20/20、历史顺序用例 8/8；Linux 用**官方 `node:20` / `node:22` 镜像**（`linux/arm64`，Debian 12）各跑完整 **8 条命令**（含 `npm run build` 与 `npm run demo:recovery`）全部 exit 0，完整套件同为 **59 套件 / 1215 用例（1214 通过、1 跳过 = darwin-only 的 ACL 用例）**，两个 Node 版本数字逐位一致。**Windows 本机无执行能力，仍未实测**；CI 对本轮**全部未推送提交**也**未运行**（`git rev-list --count origin/main..HEAD` = 30，远端仍停在 `a527c7e`）。逐项数字、通道差异与"模拟/适配器契约覆盖 ≠ 真实 DACL 验证"的区分见 `docs/verification.md` 的 "Platform results, continuation round" 一节 |
