...
| --- | --- | --- |
| C0 environment and reproduction baseline | done, one sub-item with no artefact | Baseline state is recorded in this file. The clean-install question is settled on the official `node:20` / `node:22` images below: `npm ci` completed and the following `tsc` build succeeded on both, which is only possible if the platform-specific esbuild binary was installed — the optional package was not additionally inspected by name. The worker handshake item is `0da851b`: every unexpected exit prints captured, redacted stderr, and `error` / `close` / signal / watchdog / parent-timeout are all handled; contention tests no longer guess with a 250 ms sleep. The blocked `clean` command was solved through the approved deletion mechanism rather than by editing `scripts/clean.mjs` or by relabelling a bare `tsc` as a full build. Baseline test runs were saved before the changes. |
...
| C10 clean install, platforms, final review | **partially done** — Linux and macOS measured, Windows not; CI not run | Thresholds unchanged at 75 / 70 / 65 / 78. The write-dispatch scan is pinned as the test named above rather than left as a one-off grep. Independent review was performed on the diffs and tests, not on reports. What is missing is execution evidence: no Windows host, and no CI run for the round. |
...
| 8 | 路径、owner、ACL、跨进程/同进程等待 | satisfied on POSIX and macOS; **not executed on Windows** | Path and ownership: `tests/write-state-security.test.ts:144`, `:157`, `:185`, `:195`, `:206`, `:218`, `:230`, `:241`, `:254`, `:268`. Real macOS ACL: `:282`. Cross-process exclusion: `tests/write-operation-lock.test.ts:191`; bounded wait `:227`; no TTL preemption `:349`; non-owner cannot release `:365`; same-process wait `:443` | the Windows row of this scenario rests on adapter-contract coverage only (`tests/windows-acl-adapter.test.ts`, `jest.mock('node:child_process')`, executed on macOS), and the foreign-owner case is staged by injecting `lstat` because this host cannot create a file owned by another user (`write-state-security.test.ts:157`) |
...
| 16 | 全量与产物 | **partial** | 18 tools: `tests/mcp.test.ts:14` asserts `listTools()` equals `tests/fixtures/mcp-tools-baseline.json` (18 entries), and `scripts/verify-distribution.mjs:64` reports `tools: 18`. No private files: the forbidden-content regex at `scripts/audit-package.mjs:34`, invoked by `pack:smoke` and CI | three of the four sub-clauses have no jest product: the tool-count equality compares against a fixture in this repository, "no private files" is a package script only, "the standard commands really pass" is log evidence outside the repository, and "three platforms explicit" is macOS plus a Linux container — Windows is not run |
